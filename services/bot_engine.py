...
from services.database import db_save_signal, db_get_signals, db_delete_signals_by_user, db_update_signal
        ...
        try:
            ...
            for sig in old_signals:
                ...
                if not sig.get("entry_hit") and age_ms > 1800000:
                    db_update_signal(sig["id"], {
                        "status": "CANCELLED",
                        "result": "CANCELLED",
                        "reason": "Stale signal cleared on bot restart"
                    })
                    cleared += 1
