...
class BotEngine:
    ...
    def _rebuild_counters(self):
        """
        Rebuild in-memory counters dari DB.
        SELALU reset counter dulu sebelum hitung ulang supaya
        tidak double-count kalau dipanggil lebih dari sekali.
        """
        try:
        ...
        for s in all_signals:
            ...
            # ── Win: TP atau AI_CLOSE dengan pnl >= 0 ──
            if result == "TP" or (result == "AI_CLOSE" and (s.get("pnl_pct") or 0) >= 0):
            ...
            # ── Loss: SL atau AI_CLOSE dengan pnl < 0 ──
            elif result == "SL" or (result == "AI_CLOSE" and (s.get("pnl_pct") or 0) < 0):
                self.state["trade_count"] += 1
            ...
            # ── Masih OPEN → masuk active list ──
            elif status == "OPEN":
                self.state["signals"].append(s)

            # result TIMEOUT / INVALIDATED / CANCELLED → skip (bukan trade)
    ...
    async def start(self, config: dict):
        ...
        # ── PROCESS OPEN SIGNALS FROM PREVIOUS SESSION ──
        try:
            ...
            for sig in old_signals:
                ...
                entry_hit = bool(sig.get("entry_hit"))

                if entry_hit:
                    # Signal sudah entry → resume monitor
                    if not self._find_signal(sig["id"]):
                        self.state["signals"].insert(0, sig)
                    self._active_ids.add(sig["id"])
                    asyncio.create_task(self._monitor_signal(sig["id"]))
                    resumed += 1

                elif age_ms > 1800000:
                    # Pending > 30 menit, belum entry → cancel
                    ...
                    # Pending < 30 menit, belum entry → resume monitor (masih fresh)
                    if not self._find_signal(sig["id"]):
                        self.state["signals"].insert(0, sig)
                    self._active_ids.add(sig["id"])
                    asyncio.create_task(self._monitor_signal(sig["id"]))
            ...
            if resumed > 0:
                print(f"[BotEngine] Resumed {resumed} active (entry_hit) signals")
            if pending_resumed > 0:
                print(f"[BotEngine] Resumed {pending_resumed} pending (waiting entry) signals")

            # Rebuild counters SETELAH proses signal lama
            # _rebuild_counters() reset & hitung ulang dari DB supaya tidak double count
            self._rebuild_counters()

        except Exception as e:
            print(f"[BotEngine] Could not process old signals: {e}")
    ...
    async def stop(self):
        ...
        print("[BotEngine] Scanner stopped. Active signal monitors still running.")
        return {"ok": True}

    async def shutdown(self):
        """
        Full shutdown — dipanggil saat app mati (Railway restart/deploy).
        Stop scanner + cleanup resource. Monitor tasks akan mati sendiri
        karena event loop ikut mati.
        """
        await self.stop()
        ...
        while self.running:
            try:
                ...
                for sym in pool:
                    ...
                    # ── STEP 1: Pre-filter via Qwen AI (cepat, 1 token) ──────────
                    print(f" [Pre-filter] Qwen AI scanning {sym}…")
                    pre_signal = await qwen_ai.analyze(sym, candles_by_tf, current_price)
                    pre_decision = pre_signal.get("decision", "NO TRADE")

                    if pre_decision == "NO TRADE":
                        # Qwen bilang NO TRADE → skip world, hemat waktu & token
                        print(f" [Pre-filter] {sym} → NO TRADE (skipping Trading World)")
                        self.state["no_trade_count"] += 1
                        await asyncio.sleep(INTER_SYMBOL_DELAY)
                        continue

                    # ── STEP 2: Lolos pre-filter → masuk Trading World ───────────
                    print(
                        f" [Pre-filter] {sym} → {pre_decision} (conf={pre_signal.get('confidence')}%) "
                        f"— sending to Trading World…"
                    )
                    try:
                        world_result = await trading_world.run_simulation(
                            sym, candles_by_tf, current_price
                        )
                        # Ambil trend & pattern dari agent round 3 yang sesuai final decision
                        r3_opinions = world_result.round3_opinions or []
                        majority_opinion = next(
                            (o for o in r3_opinions if o.decision == world_result.final_decision),
                            r3_opinions[0] if r3_opinions else None,
                        )
                        signal = {
                            "trend":       majority_opinion.trend if majority_opinion else "SIDEWAYS",
                            "pattern":     majority_opinion.pattern if majority_opinion else "",
                            "decision":    world_result.final_decision,
                            "entry":       world_result.consensus_entry,
                            "tp1":         world_result.consensus_tp1,
                            "tp2":         world_result.consensus_tp2,
                            "tp_max":      world_result.consensus_tp_max,
                            "tp":          world_result.consensus_tp1,
                            "sl":          world_result.consensus_sl,
                            "invalidation": None,
                            "reason": (
                                f"[World consensus {world_result.consensus_confidence:.1f}%"
                                f" / {world_result.agent_count} agents] "
                                + (majority_opinion.reason[:150] if majority_opinion else "")
                            ),
                            "confidence":  int(world_result.consensus_confidence),
                        }
                    except Exception as e:
                        logger.error(f"Trading world error for {sym}: {e}", exc_info=True)
                        signal = {
                            "decision": "NO TRADE",
                            "reason": f"World simulation error: {e}",
                            "confidence": 0,
                            "trend": "SIDEWAYS", "pattern": "none",
                            "entry": None, "tp1": None, "tp2": None,
                            "tp_max": None, "tp": None, "sl": None,
                            "invalidation": None,
                        }
    ...
    async def _monitor_signal(self, sig_id: str):
        ...
        while time.time() - start < max_duration:
            # Catatan: self.running = False artinya SCANNER sudah stop,
            # tapi monitor ini tetap harus jalan sampai signal closed.
            # Yang break monitor hanya: signal closed/invalidated, timeout 8h,
            # atau task di-cancel (shutdown Railway).
