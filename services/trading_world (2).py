...
async def _kirim_pesan(
    ...
    # Build content — array format sama seperti gateway (image_url + text)
    if charts:
        content: Any = []
        ...
        # Fallback: plain string kalau tidak ada charts
        content = full_text
...
class TradingWorldSimulation:
    ...
    async def run_simulation(self, symbol, candles_by_tf, current_price,
        ...
        # Build charts (text only untuk reverse API, tapi tetap build untuk internal use)
        charts: Dict[str, str] = {}
        loop = asyncio.get_event_loop()
        for tf in ["5m", "15m", "1h", "4h"]:
            if tf in candles_by_tf:
                img = await loop.run_in_executor(None, _draw_chart, candles_by_tf[tf], symbol, tf)
                if img:
                    charts[tf] = img
