...
You will receive:
 1. POSITION TIMING: entry time (TIME ENTRY), current time (TIME NOW), and how long the trade has been open (TIME ELAPSED)
 ...
 4. POSITION STATUS: entry price, TP, SL, current price, margin (USDT), leverage, notional value, and live unrealized PnL
 5. BOT PERFORMANCE STATS: total trades ever, total wins, total losses, and overall win rate %
 6. LAST 10 CLOSED TRADES: recent trade outcomes with PnL and close reasons
 7. Latest candle data and charts

TIME AWARENESS:
- "Time Elapsed" tells you how long this trade has been alive. A trade that just opened has barely had time to develop.
...
- Price naturally moves against a new position briefly before reaching TP.
  Do NOT mistake normal volatility for thesis invalidation, especially on young positions.

CONFIDENCE RULES:
- Be decisive. HOLD = thesis is intact. CLOSE = thesis is clearly broken.
- Do NOT flip decisions every cycle. Require structural change (new low for LONG / new high for SHORT), not just price drift, to go from HOLD to CLOSE.
- Use the win rate and history to calibrate confidence in the system's entries. A high win rate means entries are generally well-timed — be patient.
- The margin and leverage tell you exact financial exposure: a 1% price move = leverage × margin in PnL impact. Use this for context, not fear.
- Review the last 10 trades for patterns: if the bot has been consistently right recently, trust it. Do not cut winners short based on noise.
...
def _safe_float(val, default: float = 0.0) -> float:
    """Convert val to float safely — returns default for None, NaN, inf, or unparseable."""
    try:
        f = float(val)
        import math
...
def _safe_ts(ts) -> Optional[int]:
    """Convert a timestamp to int ms, handling None/NaN/0/string."""
    if ts is None:
        return None
    try:
        import math
...
class PositionAIClient:
    ...
    async def decide(
        ...
        tp            = _safe_float(tp)
        sl            = _safe_float(sl)
        leverage      = _safe_int(leverage, default=1)
        margin_usdt   = _safe_float(margin_usdt)
        opened_at     = _safe_ts(opened_at)

        # Guard against zero entry/price (would cause division-by-zero)
        if entry <= 0:
        ...
        if direction == "LONG":
            ...
            pct_to_tp = round((tp - current_price) / current_price * 100, 3) if current_price > 0 else 0.0
            pct_to_sl = round((current_price - sl) / current_price * 100, 3) if current_price > 0 else 0.0
        else:
            pnl_pct = round((entry - current_price) / entry * 100, 3)
            pct_to_tp = round((current_price - tp) / current_price * 100, 3) if current_price > 0 else 0.0
            pct_to_sl = round((sl - current_price) / current_price * 100, 3) if current_price > 0 else 0.0
        ...
        # ── Build Win Rate / Performance Stats block ──────────────────
        winrate_block = ""
        # Compute win_rate from totals if not explicitly provided
        if win_rate is None and total_wins is not None and total_losses is not None:
            _denom = (total_wins or 0) + (total_losses or 0)
            if _denom > 0:
                win_rate = round((total_wins / _denom) * 100, 1)
        # Compute total_trades if not provided
        if total_trades is None and total_wins is not None and total_losses is not None:
            total_trades = (total_wins or 0) + (total_losses or 0)

        _tt  = total_trades  if total_trades  is not None else "N/A"
        _tw  = total_wins    if total_wins    is not None else "N/A"
        _tl  = total_losses  if total_losses  is not None else "N/A"
        _wr  = f"{win_rate:.1f}%" if win_rate is not None else "N/A"
        if win_rate is not None:
            wr_label = "🟢 Strong" if win_rate >= 60 else ("🟡 Average" if win_rate >= 45 else "🔴 Weak")
        else:
            wr_label = ""
        winrate_block = (
            ...
            f" Total Wins   : {_tw}\n"
            f" Total Losses : {_tl}\n"
            f" Win Rate     : {_wr} {wr_label}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        )

        # ── Build Trade History block ──────────────────────────────────
        history_block = ""
        if trade_history:
            ...
            for i, t in enumerate(trades):
                ...
                pnl_u  = t.get("pnl_usdt", t.get("pnl", None))
                pnl_p  = t.get("pnl_pct", None)
                close_reason = t.get("close_reason", t.get("reason", ""))
                closed_at = t.get("closed_at", t.get("close_time", None))
                ts_str = _fmt_ts(closed_at) if closed_at else ""

                # Determine win/loss icon
                is_win = (
                    str(result).upper() in ("WIN", "TP", "TP1", "TP2", "TAKE_PROFIT")
                    or (pnl_u is not None and float(pnl_u) > 0)
                    or (pnl_p is not None and float(pnl_p) > 0)
                )
                icon = "✅" if is_win else "❌"
                if is_win:
                    wins += 1
                ...
                if pnl_u is not None:
                    pnl_str += f" PnL: {'+' if float(pnl_u)>=0 else ''}{float(pnl_u):.2f} USDT"
                if pnl_p is not None:
                    pnl_str += f" ({'+' if float(pnl_p)>=0 else ''}{float(pnl_p):.2f}%)"

                reason_str = f" | {close_reason}" if close_reason else ""
                ts_str2 = f" | {ts_str}" if ts_str else ""
                lines.append(f" {icon} #{len(trades)-i} {sym} {side}{pnl_str}{reason_str}{ts_str2}")

            recent_label = f"{wins}W/{losses}L in last {len(trades)}"
            streak = ""
            # Detect streak (last N same result)
            if trades:
                last_result = None
                streak_len = 0
                for t in reversed(trades):
                    pnl_u = t.get("pnl_usdt", t.get("pnl", None))
                    pnl_p = t.get("pnl_pct", None)
                    r = t.get("result", t.get("outcome", ""))
                    cur = (
                        str(r).upper() in ("WIN", "TP", "TP1", "TP2", "TAKE_PROFIT")
                        or (pnl_u is not None and float(pnl_u) > 0)
                        or (pnl_p is not None and float(pnl_p) > 0)
                    )
                    if last_result is None:
                        last_result = cur
                        streak_len = 1
                    elif cur == last_result:
                        streak_len += 1
                    else:
                        break
                if streak_len >= 2:
                    label = f"{'WIN' if last_result else 'LOSS'}"
                    streak = f" | 🔥 Current streak: {streak_len}x {label}"

            lines.append(f" Recent summary: {recent_label}{streak}")
            lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
            history_block = "\n".join(lines)
        ...
        user_text = (
            ...
            f" Notional Value: {round(margin_usdt * leverage, 2)} USDT\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"{chr(10).join(ohlcv_blocks)}\n\n"
            f"Charts attached above.\n"
            f"⏱ This position has been open for {elapsed_str}.\n"
            f"📊 Bot win rate: {_wr} over {_tt} total trades ({_tw} wins / {_tl} losses).\n"
