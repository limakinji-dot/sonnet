    ...
    >
      <div ref={contentRef} className="w-full max-w-5xl mx-auto">
        ...
        <div className="glass-gold rounded-2xl border border-[#d4a847]/15 overflow-hidden">
          ...
          {/* Neural canvas */}
          <div className="relative bg-black/40 border-b border-white/[0.04]" style={{ aspectRatio: "16/9", maxHeight: 360 }}>
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          {[
            ...
            { label: "Bearer Tokens", value: "5" },
            { label: "Deliberation Rounds", value: "3" },
          ].map((stat) => (
            <div key={stat.label} className="glass rounded-xl p-3 sm:p-4 text-center border border-white/[0.05]">
              <div className="text-lg sm:text-xl font-mono font-bold text-white/80 mb-1">{stat.value}</div>
              <div className="text-[8px] font-mono text-white/25 tracking-widest">{stat.label.toUpperCase()}</div>
            </div>
          ))}
        </div>
      </div>
